...
- `kpi_validation_checks.sql` checks KPI row counts, impossible KPI values, divide-by-zero handling, missing targets, and carried-forward staging quality flags.
- `kpi_reconciliation_checks.sql` reconciles staging totals to KPI fact and summary totals.

Run Phase 1 validation after `sql/01_create_raw_tables.sql` and `sql/02_insert_sample_data.sql` are complete. Run Phase 2 validation after `sql/03_create_staging_tables.sql` is complete. Run Phase 3 validation after `sql/05_kpi_calculations.sql` is complete.
